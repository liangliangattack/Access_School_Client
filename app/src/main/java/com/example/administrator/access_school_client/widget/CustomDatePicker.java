package com.example.administrator.access_school_client.widget;
...
public class CustomDatePicker {
    ...
    private TextView tv_cancle, tv_select, month_text, day_text, hour_text, minute_text;
    ...
    public CustomDatePicker(Context context, ResultHandler resultHandler, String startDate, String endDate) {
        if (isValidDate(startDate, "yyyy-MM-dd HH:mm") && isValidDate(endDate, "yyyy-MM-dd HH:mm")) {
            ...
            // 通过 Instance 反射创建 calendar 实例对象
            selectedCalender = Calendar.getInstance();
            // 同上 创建 startCalendar 实例对象
            startCalendar = Calendar.getInstance();
            // 创建 endCalendar 实例对象
            endCalendar = Calendar.getInstance();
            // 设置日期格式, Locale.CHINA 设置中国北京时间
    ...
    private void initDialog() {
        if (datePickerDialog == null) {
            ...
            /*
                得到对话框的 window 对象, 并将对话框放入 window 底部
             */
    ...
    public void showSpecificTime1(boolean show) {
        if (canAccess) {
            if (show) {
                disScrollUnit();
                hour_pv.setVisibility(View.VISIBLE);
